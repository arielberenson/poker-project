...
class PlayerDisplay:
    def __init__(self, text, chips, padding=5):
        ...
        self.chips = chips
        self.text = text
        image_path = os.path.join(os.path.dirname(__file__), '..', 'images', 'cardsback.png')
...
class PlayersDisplay:
    ...
    def add_players(self, data):
        for players in data:
            self.players.append([players[0], PlayerDisplay(players[0], str(players[1]))])
    ...
    def remove_player(self, name):
        for player in self.players:
            if player[0] == name:  # Assuming player[1] is the name
    ...
    def draw(self, screen):
        if len(self.players) == 1:
            for element in self.players:
                element[1].draw(screen, sw * 0.1, sh * 0.6)
        elif len(self.players) == 2:
            self.players[0][1].draw(screen, sw * 0.1, sh * 0.6)
            self.players[1][1].draw(screen, sw * 0.6, sh * 0.6)

    def get_player(self, player):
        for name, visual in self.players:
            if name == player:
                return visual
